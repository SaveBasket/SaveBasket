...
create table if not exists public.subscriptions(id uuid primary key default gen_random_uuid(), user_id uuid references auth.users(id) on delete cascade, provider text not null, provider_subscription_id text unique, plan text not null, status text not null, current_period_end timestamptz, stripe_customer_id text, raw_event_id text, created_at timestamptz not null default now());
